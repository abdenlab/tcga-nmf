...
class NMFHeatmapWidget(anywidget.AnyWidget):
    ...
    selected_sample_ids_from_heatmap = List(
        [], help="List of sample IDs selected from the NMF heatmap."
    ).tag(sync=True)
    ...
    def __init__(self, cfg_path: str | Path = "config.json"):
        ...
        self.figure = go.FigureWidget(
            create_empty_placeholder_figure("Initializing...")
        )
        self.k_dropdown = widgets.Dropdown(
            options=[("Loading...", "")],
            value="",
            description="NMF K Value:",
            disabled=True,
        )
        ...
        try:
                ...
                if default_csv_full_path:
                    ...
                    self.figure = go.FigureWidget(
                        create_empty_placeholder_figure(
                            "No NMF data files found in specified directory. Please check config.json and data/comps folder."
                        )
                    )
                    self.widget_layout.children = [
                        self.figure
                    ]  # No controls if no data
            ...
            traceback.print_exc()
            self.figure = go.FigureWidget(
                create_empty_placeholder_figure(f"Initialization Error: {e}")
            )
    ...
    def _on_k_change(self, change):
        ...
        if new_k_file != self._current_k_file:
            ...
            k_value_display = next(
                (
                    k_file["k_value"]
                    for k_file in self.k_files
                    if k_file["filename"] == new_k_file
                ),
                "Unknown",
            )
    ...
    def _recreate_figure(self):
        ...
        try:
            H_matrix, sample_ids_for_current_k, cancer_types = get_prepared_data(
                self._cfg_path, k_filename=self._current_k_file
            )
            ...
            traceback.print_exc()
            self.figure = go.FigureWidget(
                create_empty_placeholder_figure(f"Figure Creation Error: {e}")
            )
    ...
    def _on_plotly_selection(self, trace, points, state):
        ...
        if points and points.point_inds:
            ...
            original_indices_selected = self._current_heatmap_sample_order_indices[
                selected_x_indices_on_heatmap
            ]

            selected_ids_from_heatmap_view = np.array(
                self._all_original_sample_ids_for_current_k
            )[original_indices_selected].tolist()

        if sorted(selected_ids_from_heatmap_view) != sorted(
            self.selected_sample_ids_from_heatmap
        ):
            self.selected_sample_ids_from_heatmap = selected_ids_from_heatmap_view
            if selected_ids_from_heatmap_view:
                print(
                    f"NMFHeatmapWidget: Selection made on heatmap: {len(selected_ids_from_heatmap_view)} samples."
                )
            else:
    ...
    def update_highlight_from_external(self, sample_ids_to_highlight: list[str]):
        ...
        if sorted(self._highlighted_sample_ids) != sorted(sample_ids_to_highlight):
            print(
                f"NMFHeatmapWidget: Updating highlight for {len(sample_ids_to_highlight)} samples from external source."
            )
