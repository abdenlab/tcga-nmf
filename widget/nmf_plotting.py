...
def create_nmf_heatmap_figure(
    ...
    """
    # --- load data ------------------------------------------------------ #
    H, sample_ids_from_file, cancer_types = get_prepared_data(
        cfg_path, k_filename=k_filename
    )
    ...
    _add_component_strip_with_legend(
    ...
    if selected_sample_ids:
        _add_highlight_layer(
            fig,
            all_sample_ids,
            current_heatmap_samp_order_indices,
            selected_sample_ids,
            n_comps,
        )
...
def _add_proportional_bar_chart(
    fig: go.Figure, H_sorted: np.ndarray, comp_colors: list, comp_order: np.ndarray
) -> None:
    _, _ = H_sorted.shape
...
def _add_component_strip_with_legend(
    fig: go.Figure,
    H_ord: np.ndarray,
...
def _add_highlight_layer(
    fig: go.Figure,
    all_sample_ids_original_order: list[str],
    current_heatmap_samp_order_indices: np.ndarray,
    ...
    is_selected_in_original_order = np.array(
        [sid in selected_sample_ids for sid in all_sample_ids_original_order]
    )

    is_selected_on_heatmap = is_selected_in_original_order[
        current_heatmap_samp_order_indices
    ]
