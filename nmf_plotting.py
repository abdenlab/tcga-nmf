...
def create_nmf_heatmap_figure(
    cfg_path: str | Path = "config.json", sort_method: str = "component"
) -> go.Figure:
    """
    ...
    _add_component_strip_with_legend(
        fig, H_ord, samp_order, comp_colors, n_comps, comp_order
    )

    # 4) Cancer type strip with legend
    _add_cancer_strip_with_legend(
        fig, cancer_types, samp_order, uniq_cancers, cancer_color_map
    )
...
def _add_main_heatmap(
    fig: go.Figure, H_sorted: np.ndarray, comp_order: np.ndarray
) -> None:
    ...
    for i, comp_idx in enumerate(comp_order):
        ...
        fig.add_trace(
            go.Bar(
                ...
                marker_color=color,
                hovertemplate="Sample: %{x}<br>Component: Comp {comp_idx+1}<br>Proportion: %{y:.2f}<extra></extra>",
            ),
            row=2,
            col=1,
        )
